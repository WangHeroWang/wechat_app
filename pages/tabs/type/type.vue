<template>
	<view class="">
		<view class="container">
			...
			<view  class="index-List-box" v-for="(item,index) in stationList" :key="index" @click="toggle(item.staticId)">
...
<script>
	...
	export default {
		...
		methods: {
			...
			getStationList() {
				uni.request({
					url: this.serverUrl + '/api/wx/filling/static/nearby/list', //仅为示例，并非真实接口地址。
					data: {
						lng: this.lng,
						lat: this.lat,
					},
